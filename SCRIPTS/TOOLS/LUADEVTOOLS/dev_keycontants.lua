...
local col2 = this.TX.COLOR and 200 or 84
local col3 = this.TX.COLOR and 250 or 104
local lines = this.TX.COLOR and (this.TX.NV14 and 24 or 14) or 7
local swp = -1024
local viewType = 1
local view = constants[viewType]
local firstLine = 1
local lastLine = #view-1
lines = math.min(lines,lastLine)
...
local function run(e)
  ...
  this.GUI.drawTopBar()
  lcd.drawText( col1, 0, 'Constatnt name', this.GUI.C2+SMLSIZE)
  lcd.drawText( col2, 0, 'Value', this.GUI.C2+SMLSIZE)

  for i=firstLine,lastLine do
    local y = (i-firstLine+1)*this.GUI.lineHeight
    lcd.drawText( col0, y+2, i, this.GUI.C2+SMLSIZE+RIGHT)
    lcd.drawText( col1, y+2, view[i][1], this.GUI.C1+SMLSIZE)
    lcd.drawText( col2, y+2, view[i][2] or 'nil', this.GUI.C1+SMLSIZE)
    lcd.drawText( col3, y+2, view[i][2] and string.format('%04X',view[i][2]) or 'nil', this.GUI.C1+SMLSIZE)
  end

  if e == EVT_VIRTUAL_NEXT then
    firstLine = math.min(lastLine-lines+1,firstLine + 1)
  elseif e == EVT_VIRTUAL_PREV then
    firstLine = math.max(1,firstLine - 1)
  end

  local sw = getValue('sa')
  if sw ~= swp then
    viewType = sw >= 0 and 2 or 1
